...
class Solution:
    ...
    def findLadders(self, start, end, dictonary):
        ...
        dictonary.add(end)
        distance = {}
        self.bfs(end, start, distance, dictonary)
        res = []
        self.dfs(start, end, distance, dictonary, [start], res)
        return res
        
    def get_nextWords(self, word, dictonary):
        words = []
        for i in range(len(word)):
            for char in list(string.ascii_lowercase):
                next_words = word[:i] + char + word[i+1:]
                if next_words != word and next_words in dictonary:
                    words.append(next_words)
        return words
                
    def bfs(self, start, end, distance, dictonary):
        distance[start] = 0
        queue = deque([start])
        while queue:
            word = queue.popleft()
            for next_word in self.get_nextWords(word, dictonary):
                if next_word not in distance:
                    distance[next_word] = distance[word] + 1 
                    queue.append(next_word)
    
    def dfs(self, cur, target, distance, dictonary, path, res):
        # quit situation
        if cur == target:
            res.append(list(path))
            return
        for word in self.get_nextWords(cur, dictonary):
            if distance[word] != distance[cur] - 1:
                continue
            path.append(word)
            self.dfs(word, target, distance, dictonary, path, res)
            path.pop()
...
if __name__ == '__main__':
    ...
    startWords = "sand"
    endWords = "acne"
    wordLists = ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel", "send",
    ...
    print(sol.findLadders(startWords, endWords, set(wordLists)))
